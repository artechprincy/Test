...
if (!function_exists('get_routines_by_day_time')) {

    function get_routines_by_day_time($time_table_id, $day, $time)
    {
        $ci = &get_instance();

        $ci->db->select('t1.*, t2.name as subject_name, t3.name as teacher_name');
        $ci->db->from('routines AS t1');
        $ci->db->join('subjects AS t2', 't2.id = t1.subject_id', 'left');
        $ci->db->join('teachers AS t3', 't3.id = t1.teacher_id', 'left');
        $ci->db->where('t1.time_table_id', $time_table_id);
        $ci->db->where('t1.day', $day);
        $ci->db->where('TIME(t1.start_time) >=', $time);
        // $ci->db->where('TIME(t1.end_time) <=', $time);
        $query = $ci->db->get();
        ...
        if (!empty($row)) {
            ...
            $row->diff = round(abs($from_time - $to_time) / 60, 2);
        }
        return $row;
    }
}
...
if (!function_exists('get_total_student_by_section')) {
...
if (!function_exists('check_enrollments')) {
    function check_enrollments($class_id, $section_id)
    {
...
if (!function_exists('check_saas_status')) {
    ...
    function check_saas_status($school_id, $feature = null, $role = 0)
    {
...
if (!function_exists('get_guardian_access_data')) {
    ...
    function get_guardian_access_data($type = NULL)
    {
        ...
        $school_id = $ci->session->userdata('school_id');
        $guardian_id = $ci->session->userdata('id');
        ...
        $ci->db->join('classes AS C', 'C.id = E.class_id', 'left');
        $ci->db->join('sections AS SE', 'SE.id = E.section_id', 'left');
        $ci->db->join('guardians AS GU', 'GU.id = S.guardian_id', 'left');
        $ci->db->where('E.academic_year_id', $school->academic_year_id);
        $ci->db->where('GU.user_id', $guardian_id);
...
if (!function_exists('get_teacher_access_data')) {
    ...
    function get_teacher_access_data($type = NULL)
    {
        ...
        $ci->db->select('T.class_id');
        $ci->db->from('routines AS R');
        $ci->db->join('time_table AS T', 'T.id = R.time_table_id', 'left');
        $ci->db->where('R.teacher_id', $ci->session->userdata('profile_id'));
