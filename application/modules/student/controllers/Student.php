...
class Student extends MY_Controller
{
    ...
    public function index($class_id = null)
    {
        ...
        $this->data['row'] = $this->db->get_where('schools', ['id' => $condition['school_id']])->row();

        $last_students = $this->db->where('school_id', $condition['school_id'])->order_by('id', "desc")->limit(1)->get('students')->row();
        if (!empty($last_students)) {
            $this->data['get_last_id'] = sprintf('%04d', trim($last_students->admission_no, $this->data['row']->admission_no) + 1);
        } else {
            $this->data['get_last_id'] = '001';
        }
        $this->data['schools'] = $this->schools;
        // _d($this->data['row']);die;
        $this->data['list'] = TRUE;
    ...
    private function _prepare_student_validation()
    {
        ...
        if ($this->input->post('is_guardian') != 'exist_guardian' && !$this->input->post('id')) {
            $this->form_validation->set_rules('gud_username',   $this->lang->line('username'), 'trim|required|callback_username');
        ...
        $this->form_validation->set_rules('birth_certificate', $this->lang->line('photo'), 'trim|callback_birth_certificate');
        ...
        if ($this->input->post('gud_username') == $this->input->post('username')) {
    ...
    public function gud_username()
    {
        if ($this->input->post('gud_username') == $this->input->post('username')) {
            $this->form_validation->set_message('gud_username', $this->lang->line('already_exist'));
            return FALSE;
        }
        if ($this->input->post('id') == '') {
            $gud_username = $this->student->duplicate_check($this->input->post('gud_username'));
            if ($gud_username) {
                $this->form_validation->set_message('gud_username', $this->lang->line('already_exist'));
                return FALSE;
            } else {
            ...
            $gud_username = $this->student->duplicate_check($this->input->post('gud_username'), $this->input->post('id'));
            if ($gud_username) {
                $this->form_validation->set_message('gud_username', $this->lang->line('already_exist'));
                return FALSE;
            } else {
    ...
    public function birth_certificate()
    {
        if ($_FILES['birth_certificate']['name']) {
            ...
            $ext = pathinfo($name, PATHINFO_EXTENSION);
            // file,jpg,jpeg,png,doc,docx,pdf
            if ($ext == 'jpg' || $ext == 'jpeg' || $ext == 'png' || $ext == 'pdf' || $ext == 'doc' || $ext == 'docx') {
                return TRUE;
            } else {
                $this->form_validation->set_message('birth_certificate', $this->lang->line('select_valid_file_format'));
                return FALSE;
            }
        }
    }
    ...
    private function _upload_birth_certificate()
    {

        $prev_photo = $this->input->post('prev_birth_certificate');
        $photo = $_FILES['birth_certificate']['name'];
        $photo_type = $_FILES['birth_certificate']['type'];
        $return_photo = '';
        if ($photo != "") {
            if (
                $photo_type == 'image/jpeg' || $photo_type == 'image/pjpeg' ||
                $photo_type == 'image/jpg' || $photo_type == 'image/png' ||
                $photo_type == 'image/x-png' || $photo_type == 'application/pdf' ||
                $photo_type == 'application/msword'
            ) {

                $destination = 'assets/uploads/student-birth-certificate/';

                $file_type = explode(".", $photo);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $photo_path = 'birth-certificate-' . time() . '-sms.' . $extension;
    ...
    private function _upload_transfer_certificate()
    {
        ...
        if ($transfer_certificate != "") {
            if (
                $transfer_certificate_type == 'image/jpeg' || $transfer_certificate_type == 'image/pjpeg' ||
