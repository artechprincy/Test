...
class Application extends MY_Controller
{
    ...
    private function _prepare_application_validation()
    {
        ...
        $this->form_validation->set_rules('leave_day', 'Leave Day', 'trim|callback_leave_day');
    ...
    {
        $school = $this->application->get_school_by_id($this->input->post('school_id'));
        ...
        $leave_type = $this->application->get_single('leave_types', array('id' => $this->input->post('type_id'), 'role_id' => $this->input->post('role_id'), 'school_id' => $this->input->post('school_id')));

        $total_need_leave = $used + $this->input->post('leave_day');
        if ($leave_type->total_leave < $total_need_leave) {
            $this->form_validation->set_message('leave_day', $this->lang->line('you_have_remain_leave') . ' : ' . ($leave_type->total_leave - $used));
            return FALSE;
        } else {
            return TRUE;
        }

        // $leave_day   = date('Y-m-d', strtotime($this->input->post('leave_day')));
        // $count = $this->application->get_total_leave($this->input->post('user_id'));
        // $total = $this->db->get_where('leave_types', ['id' => $this->input->post('type_id')])->row()->total_leave;
        // if ($total > ((int)$leave_day + (int)$count)) {
        //     $this->form_validation->set_message('leave_day', "Dont have");
        //     return FALSE;
        // } else {
        //     return TRUE;
        // }
