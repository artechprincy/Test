                ...
                <div class="" data-example-id="togglable-tabs">
                    ...
                    <ul class="nav nav-tabs bordered">
                        <li class="<?php if (isset($list)) {
                                        echo 'active';
                                    } ?>"><a href="#tab_issue_list" role="tab" data-toggle="tab" aria-expanded="true"><i class="fa fa-list-ol"></i> <?php echo $this->lang->line('list'); ?></a> </li>
                        <?php if (has_permission(ADD, 'library', 'apply')) { ?>
                            <li class="<?php if (isset($add)) {
                                            echo 'active';
                                        } ?>"><a href="#tab_add_apply" role="tab" data-toggle="tab" aria-expanded="false"><i class="fa fa-plus-square-o"></i> <?php echo $this->lang->line('apply_book'); ?></a> </li>
                        <?php } ?>
                            ...
                            <?php if ($this->session->userdata('role_id') == SUPER_ADMIN) {  ?>
                                <select class="form-control col-md-7 col-xs-12" onchange="get_book_list_by_school(this.value);">
                                    ...
                                    <?php foreach ($schools as $obj) { ?>
                                        <option value="<?php echo site_url('library/issue/index/' . $obj->id); ?>" <?php if (isset($filter_school_id) && $filter_school_id == $obj->id) {
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade in <?php if (isset($list)) {
                            ...
                            <div class="x_content">
                                <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                        <?php $count = 1;
                                        if (isset($apply_books) && !empty($apply_books)) { ?>
                                            <?php foreach ($apply_books as $obj) { ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php if (has_permission(DELETE, 'library', 'apply')) { ?>
                                                            <a href="<?php echo site_url('library/apply/delete/' . $obj->apply_id); ?>" onclick="javascript: return confirm('<?php echo $this->lang->line('confirm_alert'); ?>');" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> <?php echo $this->lang->line('delete'); ?> </a>
                                                        <?php } ?>
                                                    </td>
                                                </tr>
                                            <?php } ?>
                        ...
                        <div class="tab-pane fade in <?php if (isset($add)) {
                            ...
                            <div class="x_content">
                                <?php echo form_open_multipart(site_url('library/apply/add'), array('name' => 'add', 'id' => 'add', 'class' => 'form-horizontal form-label-left'), ''); ?>
                                            ...
                                            <?php if (isset($members) && !empty($members)) { ?>
                                                <?php foreach ($members as $obj) { ?>
                                                    <?php if ($this->session->userdata('role_id') == STUDENT) {
                                                        if (logged_in_user_id() == $obj->user_id) { ?>
                                                            <option value="<?php echo $obj->lm_id; ?>" selected><?php echo $obj->name; ?> [<?php echo $obj->custom_member_id; ?>]</option>
                                                        <?php }
                                                    } else { ?>
