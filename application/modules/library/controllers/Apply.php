...
class Apply extends MY_Controller
{
    ...
    public function index($school_id = null)
    {
        ...
        if ($this->session->userdata('role_id') != SUPER_ADMIN) {
            ...
            $this->data['books'] = $this->Apply->get_list('books', $condition);
            $school_id = $condition['school_id'];
            $this->data['members'] = $this->Apply->get_library_member_list($is_library_member = 1, $condition['school_id']);
        }

        $school = $this->Apply->get_school_by_id($school_id);

        $this->data['apply_books'] = $this->Apply->get_book_applied_list($school_id, @$school->academic_year_id);
        // print_r($this->data['apply_books']);die;
        $this->data['filter_school_id'] = $school_id;
        $this->data['schools'] = $this->schools;

        $this->data['list'] = TRUE;
        $this->layout->title($this->lang->line('manage_Apply') . ' | ' . SMS);
        $this->layout->view('apply/index', $this->data);
    }
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $insert_id = $this->Apply->insert('book_apply', $data);
                if ($insert_id) {

                    $book = $this->Apply->get_single('books', array('id' => $data['book_id']));
                    create_log('Has been applied a Book : ' . $book->title);
                    success($this->lang->line('insert_success'));
                } else {
                    error($this->lang->line('insert_failed'));
                }
                redirect('library/apply/index');
            } else {
                error($this->lang->line('insert_failed'));
            }
        }
        ...
        $this->data['schools'] = $this->schools;
        $this->data['issue_books'] = $this->Apply->get_book_issued_list();
    ...
    private function _get_posted_issue_data()
    {
        ...
        if ($this->input->post('id')) {
            ...
            $school = $this->Apply->get_school_by_id($data['school_id']);
            ...
            $data['apply_by'] = $this->session->userdata('role_id');
            $data['status'] = 1;
    ...
    public function delete($id = null)
    {
        ...
        $apply = $this->Apply->get_single('book_apply', array('id' => $id));
        if ($this->Apply->delete('book_apply', array('id' => $id))) {

            success($this->lang->line('delete_success'));
        } else {
            error($this->lang->line('delete_failed'));
        }
        redirect('library/apply/index/' . $apply->school_id);
    }
    ...
    public function get_book_by_id()
    {
        ...
        $book = $this->Apply->get_single('books', array('id' => $book_id));
    ...
    public function return_book_by_id()
    {
        ...
        $this->Apply->update('book_issues', array('is_returned' => 1, 'return_date' => $return_date), array('id' => $issue_id));
        $this->Apply->update_qty($book_id, 'return');
        echo TRUE;
    }
    ...
    public function get_book_by_school()
    {
        ...
        $books = $this->Apply->get_list('books', array('status' => 1, 'school_id' => $school_id), '', '', '', 'id', 'ASC');
    ...
    public function get_library_member_by_school()
    {
        ...
        $membrs =  $this->Apply->get_library_member_list($is_library_member = 1, $school_id);
