...
class Apply_Model extends MY_Model
{
    ...
    public function get_book_applied_list($school_id = null, $academic_year_id = null)
    {
        $this->db->select('B.*, SC.school_name, BA.id AS apply_id, BA.status as is_issue, BA.created_at as apply_date, S.name, S.photo, S.phone');
        $this->db->distinct('S.id');
        $this->db->from('book_apply AS BA');
        $this->db->join('books AS B', 'B.id = BA.book_id', 'left');
        $this->db->join('library_members AS LM', 'LM.id = BA.library_member_id', 'left');
        $this->db->join('students AS S', 'S.user_id = LM.user_id', 'left');
        $this->db->join('schools AS SC', 'SC.id = BA.school_id', 'left');

        if ($academic_year_id) {
            $this->db->where('BA.academic_year_id', $academic_year_id);
        }
