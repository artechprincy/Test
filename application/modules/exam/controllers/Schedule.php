...
class Schedule extends MY_Controller
{
    ...
    public function index($class_id = null)
    {
        ...
        if ($this->session->userdata('role_id') != SUPER_ADMIN) {
            ...
            $this->data['rooms'] = $this->schedule->get_list('school_rooms', $condition, '', '', '', 'id', 'ASC');
        }
    ...
    private function _get_posted_schedule_data()
    {
        ...
        if ($this->input->post('id')) {
            ...
            if(date('Y-m-d', strtotime($data['exam_date'])) <= date('Y-m-d')){
                error("System shouled not allow exam schedule for past");
                redirect('exam/schedule/index');
            }
