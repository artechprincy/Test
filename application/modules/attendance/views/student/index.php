                ...
                <div class="row">
                            ...
                            <select class="form-control col-md-7 col-xs-12" name="class_id" id="class_id" required="required" onchange="get_section_by_class(this.value, '');">
                                ...
                                <?php foreach ($classes as $obj) { ?>
                                    <?php if (isset($classes) && !empty($classes)) { ?>
                                        <?php if ($this->session->userdata('role_id') == TEACHER && !in_array($obj->id, $teacher_access_data)) {
                                            continue;
                                        } ?>
                    ...
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <div class="item form-group">
                            ...
                            <?php if (!empty($error)) {  ?>
                                <div class="text-danger"><?= $error; ?></div><?php
                                                                            } ?>
                        </div>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        ...
                        ?>
                            <?php foreach ($students as $obj) { ?>
                                ...
                                <tr class="checklisttr">
                                    ...
                                    <td><input type="radio" value="H" itemid="<?php echo $obj->id; ?>" name="student_<?php echo $obj->id; ?>" class="holiday fn_single_attendnce" <?php if ($attendance == 'H') {
                                                                                                                                                                                        echo 'checked="checked"';
                                                                                                                                                                                    } ?> /></td>
                                </tr>
                            <?php } ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('.fn_single_attendnce').click(function() {
            // $('input:checkbox').removeAttr('checked');
            $('#fn_present').prop('checked', false);
            $('#fn_late').prop('checked', false);
            $('#fn_absent').prop('checked', false);
        });
        ...
        $('#all_attendence').click(function() {
            $('tr.checklisttr > td > input:radio:checked').each(function() {
                ...
                statusupdate(status, student_id);
            });
            toastr.success('<?php echo $this->lang->line('update_success'); ?>');
        });
    ...
    function statusupdate(status, student_id) {
        var school_id = $('#school_id').val();
        var class_id = $('#class_id').val();
        var section_id = $('#section_id').val();
        var date = $('#date').val();
