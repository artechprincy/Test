...
class Employee extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($school_id = null) {
        ...
        if($this->session->userdata('role_id') != SUPER_ADMIN){
            ...
            $this->data['departments'] = $this->employee->get_list('departments', $condition, '', '', '', 'id', 'ASC');     
        ...
        if ($_POST) {            
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($this->input->post('role_id') == '5') {
                    $insert_id = $this->employee->insert('teachers', $data);
                }else{
    ...
    * ********************************************************** */
    private function _prepare_employee_validation() {
        ...
        $this->form_validation->set_rules('phone', $this->lang->line('phone'), 'trim|callback_phone');
    ...
    * @type            : Function
    * @function name   : phone
    * @description     : Unique check for "Employee phone" data/value                  
    *                       
    * @param           : null
    * @return          : boolean true/false 
    * ********************************************************** */ 
    public function phone() {
        if ($this->input->post('id') == '') {
            $phone = $this->employee->duplicate_check_phone($this->input->post('phone'));
            if ($phone) {
            ...
            $phone = $this->employee->duplicate_check_phone($this->input->post('phone'), $this->input->post('id'));
            if ($phone) {
                $this->form_validation->set_message('phone', $this->lang->line('already_exist'));
                return FALSE;
            } else {
